...
export default function Guide(props) {
    ...
    const channels = guidelist.map((x) => (<li key={x.channel}><a href={x.href} onClick={(e) => btnClick(e, `btn-${x.channel}`, x.href)} ><img src={x.imgSrc} /><span className="guide-number">{x.channel}</span><span className="guide-name">{x.name}</span><span className="guide-info">{x.info}</span></a></li>));
